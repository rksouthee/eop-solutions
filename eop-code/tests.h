...
void test_ch_6()
{
...

    {
        ...
        {
            pair<I, Z> p = count_n(begin(l), size(l), Z(2), Z(100));
            Assert(p.m0 == end(l) && p.m1 == Z(100) + Z(1));
        }

        {
            pair<I, N> p = count_n(begin(l), size(l), Z(2));
            Assert(p.m0 == end(l) && p.m1 == N(1));
        }

        {
            pair<I, Z> p = count_not_n(begin(l), size(l), Z(2), Z(100));
            Assert(p.m0 == end(l) && p.m1 == Z(100) + Z(5));
        }

        {
            pair<I, N> p = count_not_n(begin(l), size(l), Z(2));
        ...
        {
            pair<I, Z> p = reduce_n_nonempty(begin(l), size(l), plus<Z>());
            Assert(p.m0 == end(l) && p.m1 == Z(15));
            p = reduce_n_nonempty(begin(l), N(1), plus<Z>());
            Assert(p.m0 == successor(begin(l)) && p.m1 == Z(0));
        }

        {
            pair<int, Z> p = reduce_n(0, 0, plus<Z>(), identity<Z>(), Z(0));
            Assert(p.m0 == 0 && p.m1 == Z(0));
            p = reduce_n(0, 50, plus<Z>(), identity<Z>(), Z(0));
            Assert(p.m0 == 50 && p.m1 == Z(49*50/2));
            p = reduce_n(0, 1, plus<Z>(), identity<Z>(), Z(0));
            Assert(p.m0 == 1 && p.m1 == Z(0));
        }

        {
            pair<I, Z> p = reduce_n(begin(l), N(0), plus<Z>(), Z(0));
            Assert(p.m0 == begin(l) && p.m1 == Z(0));
            p = reduce_n(begin(l), size(l), plus<Z>(), Z(0));
            Assert(p.m0 == end(l) && p.m1 == Z(15));
            p = reduce_n(begin(l), N(1), plus<Z>(), Z(0));
            Assert(p.m0 == successor(begin(l)) && p.m1 == Z(0));
        }

        {
            pair<int, Z> p = reduce_n_nonzeroes(0, 0, plus<Z>(), identity<Z>(), Z(0));
            Assert(p.m0 == 0 && p.m1 == Z(0));
            p = reduce_n_nonzeroes(0, 50, plus<Z>(), identity<Z>(), Z(0));
            Assert(p.m0 == 50 && p.m1 == Z(49*50/2));
            p = reduce_n_nonzeroes(0, 1, plus<Z>(), identity<Z>(), Z(0));
            Assert(p.m0 == 1 && p.m1 == Z(0));
        }
        ...
        {
            triple<I, Z, N> p0 = find_mismatch(begin(l), Z(0), N(6), equal<Z>());
            Assert(p0.m0 == end(l) && p0.m1 == Z(6) && p0.m2 == N(0));
            p0 = find_mismatch(begin(l), Z(0), min(size(l), N(6)), equal<Z>());
            Assert(p0.m0 == end(l) && p0.m1 == Z(6) && p0.m2 == N(0));
            p0 = find_mismatch(begin(l), Z(0), N(4), equal<Z>());
            Assert(p0.m0 == begin(l) + N(4) && p0.m1 == Z(4) && p0.m2 == N(0));
            Z d[] = {0, 1, 2, 3, -4, 5};
            slist<Z> ld(counted_range<Z*>(d, sizeof(d)/sizeof(Z)));
            triple<I, I, N> p1 = find_mismatch(
                begin(l), begin(ld), min(size(l), size(ld)), equal<Z>());
            Assert(p1.m0 == begin(l) + N(4) && p1.m1 == begin(ld) + N(4) && p1.m2 == N(2));
        }
        {
            triple<I, N, Z> p0 = find_mismatch_n0(
                begin(l), size(l), Z(0), Z(6), equal<Z>());
            Assert(p0.m0 == end(l) && p0.m1 == N(0) && p0.m2 == Z(6));
            p0 = find_mismatch_n0(begin(l), size(l), Z(0), Z(7), equal<Z>());
            Assert(p0.m0 == end(l) && p0.m1 == N(0) && p0.m2 == Z(6));
            p0 = find_mismatch_n0(begin(l), size(l), Z(0), Z(4), equal<Z>());
            Assert(p0.m0 == begin(l) + N(4) && p0.m1 == size(l) - N(4) && p0.m2 == Z(4));
            Z d[] = {0, 1, 2, 3, -4, 5};
            slist<Z> ld(counted_range<Z*>(d, sizeof(d)/sizeof(Z)));
            triple<I, N, I> p1 = find_mismatch_n0(
                begin(l), size(l), begin(ld), end(ld), equal<Z>());
