...
template <typename I>
    requires(Readable(I) && Iterator(I))
pair<I, DistanceType(I)> find_not_n(I f, DistanceType(I) n,
                                    const ValueType(I)& x)
{
...
template <typename I, typename J>
    requires(Readable(I) && Iterator(I) && Iterator(J))
pair<I, J> count_not_n(I f, DistanceType(I) n,
                       const ValueType(I)& x, J j)
{
    // Precondition: $\func{readable\_weak\_range}(f, n)$
    while (!zero(n)) {
...
template <typename I>
    requires(Readable(I) && Iterator(I))
pair<I, DistanceType(I)> count_not_n(I f, DistanceType(I) n,
                                     const ValueType(I)& x)
{
    // Precondition: $\func{readable\_weak\_range}(f, n)$
...
template <typename I, typename Op, typename F>
    requires(Iterator(I) && BinaryOperation(Op) &&
        UnaryFunction(F) &&
        I == Domain(F) && Codomain(F) == Domain(Op))
pair<I, Domain(Op)> reduce_n_nonempty(I f,
                                      DistanceType(I) n,
                                      Op op, F fun)
{
    // Precondition: $\property{weak\_range}(f, n) \wedge n \neq 0$
    // Precondition: $\property{partially\_associative}(op)$
    // Precondition: $(\forall x \in \llbracket f, n \rrparenthesis) fun(x) \text{ is defined}$
    Domain(Op) r = fun(f);
    n = predecessor(n);
    f = successor(f);
    while (!zero(n)) {
...
template <typename I, typename Op, typename F>
    requires(Iterator(I) && BinaryOperation(Op) &&
        UnaryFunction(F) &&
        I == Domain(F) && Codomain(F) == Domain(Op))
pair<I, Domain(Op)> reduce_n(I f, DistanceType(I) n, Op op, F fun,
                             const Domain(Op)& z)
{
                                       ...
                                       Op op, F fun, const Domain(Op)& z)
{
    // Precondition: $\property{weak\_range}(f, n)$
    // Precondition: $\property{partially\_associative}(op)$
    // Precondition: $(\forall x \in \llbracket f, n \rrparenthesis) fun(x)$ is defined
    Domain(Op) x;
    do {
        if (zero(n)) return pair<I, Domain(Op)>(f, z);
        x = fun(f);
